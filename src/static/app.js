document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Reset dropdown options while preserving the placeholder
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants.length
          ? `<ul class="participants-list">${details.participants
              .map(
                (participant) =>
                  `<li><span>${participant}</span><button class="participant-delete-btn" data-activity="${encodeURIComponent(
                    name
                  )}" data-email="${encodeURIComponent(
                    participant
                  )}" type="button" aria-label="Unregister ${participant} from ${name}" title="Unregister">&#128465;</button></li>`
              )
              .join("")}</ul>`
  ...
  // Handle unregister clicks for participant delete icons
